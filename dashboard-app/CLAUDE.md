...
- 本番・開発とも PostgreSQL。開発は [docker-compose.yml](docker-compose.yml) で立ち上がる Postgres 17 コンテナを使う。初回は `docker compose up -d` → `npx prisma migrate dev`。
...
- 全ルートに `runtime = "nodejs"` と `dynamic = "force-dynamic"` を設定している。Edge ランタイムでは Prisma + libSQL アダプタが動かないため、新規ルートでも `runtime` は外さない。
