...
- **DB**: PostgreSQL via `@prisma/adapter-pg` + `pg`. `DATABASE_URL` is read at runtime in [dashboard-app/lib/db.ts](dashboard-app/lib/db.ts) and passed to `PrismaPg({ connectionString })`. Dev can point at either (a) the local `docker compose` service `m5postgres` (Postgres 17, host port **5411**) or (b) a managed Prisma Postgres — see [dashboard-app/CLAUDE.md](dashboard-app/CLAUDE.md) and `.env.example` for both URLs. Next.js reads `.env.local` with higher priority than `.env`.
- **Schema** ([dashboard-app/prisma/schema.prisma](dashboard-app/prisma/schema.prisma)): three tables — `readings` (time-series), `devices` (last-seen cache), `alert_state` (hysteresis). `readings.recorded_at` is `TIMESTAMPTZ`; `devices.last_seen_at` and `alert_state.last_fired_at` are `BIGINT` ms-epoch (kept as numeric for cheap comparisons).
...
- **Send pipeline** ([m5-co2-monitor/src/api_client.cpp](m5-co2-monitor/src/api_client.cpp)): samples are pushed onto a 900-slot ring buffer (`apiClientPushSample`). `apiClientUpdate()` drains **one sample per call from the tail (LIFO)** when Wi-Fi is up — so after a reconnect, the freshest measurement reaches the dashboard first and older queued samples backfill in reverse-chronological order. On overflow the oldest (head) is dropped. Backoff after `FAILURE_THRESHOLD` consecutive failures.
